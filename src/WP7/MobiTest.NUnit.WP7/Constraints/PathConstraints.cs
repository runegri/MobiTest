...
namespace NUnit.Framework.Constraints
{
  ...
  /// </summary>
  /// <remarks>
  /// String comparisons and ToArray(Type) were modified for Silverlight compatibility.
  /// </remarks>
  public abstract class PathConstraint : Constraint
  {
    ...
    protected string Canonicalize( string path )
    {
      ...
      return String.Join( Path.DirectorySeparatorChar.ToString(),
        (string[])parts.Cast<string>().ToArray() );
    }
    ...
    {
      StringComparison comparison = caseInsensitive ? StringComparison.InvariantCultureIgnoreCase : StringComparison.InvariantCulture;
      return string.Compare( Canonicalize( expected ), Canonicalize( (string)actual ), comparison ) == 0;
    }
    ...
    protected bool IsSamePathOrUnder( string path1, string path2 )
    {
      ...
      if( string.Compare( path1, path2.Substring( 0, length1 ), comparison ) != 0 )
        return false;
